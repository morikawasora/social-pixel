...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $iconFileName = $user['icon']; // 初期値は現在のアイコン
  ...
  if (empty($errorMessages)) {
    ...
    $successMessages[] = "プロフィール情報を更新しました。";
  }
...
<body>
  <div class="main">
    ...
    <form method="post" action="" enctype="multipart/form-data">
      ...
      <textarea id="bio" name="bio" rows="4"><?= htmlspecialchars($user['bio'] ?? '') ?></textarea><br><br>

      <label for="icon">アイコン画像:</label><br>
      <?php if (!empty($user['icon'])): ?>
        <img src="uploads/<?= htmlspecialchars($user['icon']) ?>" alt="アイコン" style="max-width:100px; max-height:100px;"><br>
      <?php else: ?>
        <p>未設定</p>
      <?php endif; ?>
